...
namespace Leashmore
{
            ...
            else
            {
                try
                {
                    ...
                    if (record_id.Length > 9)
                        show_two_line_message_in_console("Title: " + Decrypt_string_with_aes_in_cbc(Extract_value_from_record("Note", record_id, "Title")), "Content: " + Decrypt_string_with_aes_in_cbc(Extract_value_from_record("Note", record_id, "Content")));
                }
    ...
    private static bool DeleteRecord(string table, string id)
        ...
        private static string Decrypt_string_with_aes_in_cbc(string ciphertext)
        {
            ...
            {
                show_two_line_message_in_console("Failed to Decrypt Record", "Error: " + ex.Message);
